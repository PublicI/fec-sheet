...
var currencyColumns = ['beginning_balance_this_period',
...
function summaryFormat(filing_id, row, type) {
    ...
    result.push([row.committee_name, {
        v: 'ok',
        f: 'HYPERLINK("http://docquery.fec.gov/cgi-bin/forms/' + row.filer_committee_id_number + '/' +
    ...
    pg.connect(conString, function(err, client, done) {
        ...
        var stream = client
            ...
            .on('data', function(row) {
                ...
                sheet.write(_.values(_.mapValues(row, function(val, key) {
                    if (val === null) {
                        return '';
                    } else if (currencyColumns.indexOf(key) !== -1) {
